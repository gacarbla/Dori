  ...
  run: async (client, interaction) => {
    try {
      ...
      var embed = new Discord.MessageEmbed()
        .setDescription('**CUADRO DE AYUDA**\n\n**Comando `/staff comandos habilitar`**')
        .setColor(0xfee75c)
        .addField('¿Qué hace?', '¿Has desactivado una categoría en el servidor pero te arrepientes?\nNO PASA NADA, para eso tenemos este comando.')
        .addField('Uso', '`/staff comandos habilitar <módulo>`')
        .addField('Permisos bot', 'Ver canales\nEnviar mensajes', true)
        .addField('Permisos usuario', 'AMINISTRADOR', true)
